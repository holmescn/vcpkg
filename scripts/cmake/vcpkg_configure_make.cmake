...
function(vcpkg_configure_make)
    ...
    set(cm_FLAGS AS AS CCAS CC C CPP CXX FC FF GC LD LF LIBTOOL OBJC OBJXX R UPC Y RC)
    ...
    if(VCPKG_TARGET_IS_ANDROID)
		...
		if(BUILD_ARCH STREQUAL "armv7-a")
            ...
            set(ENV{CC}  armv7a-linux-androideabi$ENV{ANDROID_NATIVE_API_LEVEL}-clang)
            set(ENV{CXX} armv7a-linux-androideabi$ENV{ANDROID_NATIVE_API_LEVEL}-clang++)
		else()
        	set(arg_BUILD_TRIPLET "--build=${BUILD_ARCH}-none-linux-android")
            set(ENV{CC}  ${BUILD_ARCH}-linux-android$ENV{ANDROID_NATIVE_API_LEVEL}-clang)
            set(ENV{CXX} ${BUILD_ARCH}-linux-android$ENV{ANDROID_NATIVE_API_LEVEL}-clang++)
		endif()
        string(APPEND arg_BUILD_TRIPLET " --host=x86_64-pc-linux-gnu32")

		# export 
		foreach(_tool ${cm_TOOLS})
            string(TOLOWER "llvm-${_tool}" _llvm_tool)
            set(ENV{${_tool}} ${_llvm_tool})
		endforeach()
    endif()
